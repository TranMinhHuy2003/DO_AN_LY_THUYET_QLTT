...
-- STORED PROCEDURE
-- 1. Thống kê MAXD, TENXD, GIANHAP (nghìn đồng/lít) của các xăng dầu để các chủ cửa hàng xem
DELIMITER $$
CREATE PROCEDURE thongke_xangdau ()
BEGIN
	SELECT DISTINCT MAXD, TENXD, (GIACOSO + (THUEPHI * 1.0 / 100) * GIACOSO) AS GIANHAP  
    	FROM XANGDAU;
END$$
DELIMITER ;

-- 2. Thống kê để xem cửa hàng nào nhập xăng dầu nào, số lượng (dm3), tổng tiền (đồng)
DELIMITER $$
...
-- CRYSTAL REPORT 
-- 1. Tạo View để thống kê MAXD, TENXD, GIANHAP (nghìn đồng/lít) của các xăng dầu để các chủ cửa hàng xem
CREATE VIEW thongke_xangdau 
AS
	SELECT DISTINCT MAXD, TENXD, (GIACOSO + (THUEPHI * 1.0 / 100) * GIACOSO) AS GIANHAP  
    	FROM XANGDAU;

-- 2. Tạo View để thống kê để xem cửa hàng nào nhập xăng dầu nào, số lượng (m3), tổng tiền (đồng)
